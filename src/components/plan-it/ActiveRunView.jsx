...
const renderAssignedOrder = (
  order,
  index,
  homeZone,
  onDeleteAssignment
) => {
  const isDelivery = isPostcodeInZone(
    order.recipient_details?.postCode,
    homeZone
  );
  ...
  return (
    <Draggable
      ...
      draggableId={`assigned-order-${order.id}`}
      index={index}
    >
      {(provided) => (
        <div
        ...
        >
          ...
          <div className="planit-item-text">
            ...
            <div className="run-stats" style={{ fontSize: '0.85em', gap: '1rem' }}>
              <span>
                <Weight size={12} /> {order.cargo_details?.total_kilos || 0} kg
              </span>
              <span>
                <PalletIcon size={12} /> {order.cargo_details?.total_spaces || 0} spaces
              </span>
            </div>
          </div>
...
const ActiveRunView = ({
  ...
  return (
    <div className="card planit-section active-run-view">
      ...
      <div className="run-capacity-details" style={{ padding: '0 1rem 1rem', borderBottom: '1px solid var(--border-color)' }}>
        {run.hasCapacity ? (
          <>
        ...
        <Droppable droppableId={`run-active-${run.id}`}>
          {(provided) => (
            <div
              ...
              ref={provided.innerRef}
              className="planit-list"
            >
              {assignedOrders.map((order, index) =>
                renderAssignedOrder(order, index, homeZone, onDeleteAssignment)
              )}
...
ActiveRunView.propTypes = {
  run: PropTypes.shape({
    id: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    status: PropTypes.string,
    displayText: PropTypes.string,
    hasCapacity: PropTypes.bool,
    totalKilos: PropTypes.number,
    totalSpaces: PropTypes.number,
    maxPayload: PropTypes.number,
    maxPallets: PropTypes.number,
  }),
  assignedOrders: PropTypes.arrayOf(PropTypes.object),
